...
$url_base = "https://{$tenant}.cloud.thethings.network/api/v3/as/applications/{$application_id}/devices/";

// Check if the request is POST and contains 'payload' and 'station'
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['payload']) && isset($_POST['station'])) {
    // Get the payload and station from the AJAX request
    $input_payload = $_POST['payload'];
    $station = $_POST['station'];

    // Determine the device ID based on the station value
    $device_id = "";
    if ($station === "brgy_nueva_era") {
        $device_id = "test-id";
    } elseif ($station === "brgy_san_marcos") {
        $device_id = "lora-transceiver-2";
    } else {
    ...
    $url = $url_base . "{$device_id}/down/push";
